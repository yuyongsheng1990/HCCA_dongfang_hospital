...
## 机器学习提取的cox回归
names(df_after_ML_tran)
b.cox <- coxph(Surv(target_survival_month, target_3Y_death) ~tumor_CA19.9+DB+ALT+tumor_CEA+tumor_AFP+N+surgery_result+T+emaciation+Bismuth_C+MSKCC+Gazzaniga_T+Blumgart_T+biliary_disease+HBsAg+M+jaundice+cardio_disease,data=df_after_ML_tran, x=TRUE,y=T)
summary(b.cox)
...
b.d.cox <- coxph(Surv(target_survival_month, target_3Y_death) ~ tumor_CA19.9+DB+tumor_CEA+N+MSKCC+M, data=df_after_ML_tran, x=TRUE) 
summary(b.d.cox)
# 95%CI：c_index +/-1.96*se
CI.min <- 0.781-1.96*0.023
CI.min
CI.max <- 0.781+1.96*0.023
CI.max
...
all.d.cox <- coxph(Surv(target_survival_month, target_3Y_death) ~ jaundice+DB+ALT+tumor_CEA+tumor_CA19.9+N+M+MSKCC+Bismuth_C, data=df_after_all_tran, x=TRUE) 
summary(all.d.cox)
...
## compare Brier score
set.seed(0)
bcvBS <- pec(list("Machine Learning"=b.cox,"All Variables and Backward Stepwise"=all.d.cox,"Machine Learning and Backward Stepwise"=b.d.cox), 
...
hist(w,breaks=50,freq=T,col = "gray",main=paste("The Median of Risk Socre：",round(median(df_after_staging_tran$risk_score_3Y),1)),xlab = "The Risk Scores of Training Set",xlim=range(0,5))
x=seq(min(w),max(w),by=0.01)#做一组序列，用于绘制normal curve的x坐标
y=dnorm(x,mean(w),sd(w))#求x的正态分布函数值
lines(x,y*70,col="blue",lwd=2)#添加一条正态曲线
...
hist(w,breaks=20,freq=T,col = "gray",main=paste("The Median of Risk Socre：",round(median(df_after_staging_test$risk_score_3Y),1)),xlab = "The Risk Scores of Test Set",xlim=range(0,5))
...
df_after_staging_tran <-df_after_staging_tran[!(is.na(df_after_staging_tran$Gazzaniga_T)),]
dim(df_after_staging_tran)
# 删除测试集MSKCC分期空值
df_after_staging_test <-df_after_staging_test[!(is.na(df_after_staging_test$Gazzaniga_T)),]
dim(df_after_staging_test)
...
set.seed(0) # 设置随机数种子，不同的随机数种子，模型效果不同
compare_Cindex  <- pec::cindex(list("Risk Score System"=fit_score,"AJCC_8"=fit_ajcc, "Gazzaniga_T"=fit_gazz,"MSKCC"=fit_mskcc,"Blumgart_T"=fit_blumgart),
...
## compare 术前分期BS
set.seed(0) # 
compare_bs <- pec(list("Risk Score System"=fit_score,"AJCC_8"=fit_ajcc, "Gazzaniga_T"=fit_gazz,"MSKCC"=fit_mskcc,"Blumgart_T"=fit_blumgart),
...
print(compare_bs)
plot(compare_bs,legend.cex=1,legend.adj=0,xlim = c(0,80),xlab="Time(month)",ylab="Barier Score", legend.title=element_blank()) # legend.cex调整标签大小缩放比例;legend.col调整颜色;legend.adj标签文字相对位置;element_blank()移除所有表题
# 通过数据查看最好的随机数种子是多少,x
legend("topright", inset = 0.05, c("Risk Score System","AJCC_8","Gazzaniga_T","MSKCC","Blumgart_T"),
       lty = c(1), col = c('black','red','green','blue','skyblue'), box.lty=0) # 添加线条标签; legend去除外边框线box.lty=0
